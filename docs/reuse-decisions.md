...
| Playwright | reporter, screenshots, vidéos, trace viewer ; paquet E2E isolé `1.63.0` | Apache-2.0 | actif, multi-navigateur | **Retenu pour le Studio et les E2E.** Le reporter reste sans dépendance ; le vrai paquet n'existe que sous `e2e/`, hors workspaces racine et sans installation de navigateur implicite |
...
| `@playwright/test` | `1.63.0`, Chromium headless ou canal explicite Chrome/Edge, routes HTTP et WebSocket | Apache-2.0 | **Ajouté au paquet isolé `e2e/`.** Hors workspaces racine, navigateur installé et suite lancée uniquement par action explicite ; le parcours shell/Studio a réussi dans un Edge local |
