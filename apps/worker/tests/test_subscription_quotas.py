"""Sondes des quotas réels d'abonnement du worker (Codex CLI, Claude Code).

Aucun accès réseau et aucun compte réel : l'API est simulée par
``httpx.MockTransport`` et Codex par un vrai processus local déterministe
(``fake_codex_app_server.py``), dont les réponses sont validées contre les schémas
publiés par Codex CLI 0.156.1 (``fixtures/codex_app_server_0_156_1``).
"""
...
from acp_contracts import SubscriptionQuotaBatch, SubscriptionQuotaReport
from acp_worker import subscription_quotas as quotas
from acp_worker.config import WorkerConfig, WorkerConfigurationError
from acp_worker.executors import ExecutorConfig, ExecutorSpec
from acp_worker.local_log import WorkerLogger
from acp_worker.main import run_forever
from acp_worker.state import WorkerCredentials
from acp_worker.subscription_quotas import (
    API_KEY_VARIABLES,
...
def _clean_worker_environment(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_worker_config_carries_the_quota_settings(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    _clean_worker_environment(monkeypatch, tmp_path)
    assert WorkerConfig.from_env().subscription_quotas.enabled is False
    ...
    with pytest.raises(WorkerConfigurationError, match="ACP_WORKER_QUOTA_INTERVAL_SECONDS"):
        WorkerConfig.from_env()


def test_doctor_reports_the_quota_state_without_paths(
    ...
    offline = lambda *args, **kwargs: (_ for _ in ()).throw(httpx.ConnectError("hors ligne"))  # noqa: E731
    monkeypatch.setattr("acp_worker.cli.httpx.get", offline)
    monkeypatch.setattr("acp_worker.cli.httpx.post", offline)
...
def loop_worker_config(tmp_path: Path, quota: SubscriptionQuotaConfig) -> WorkerConfig:
    return WorkerConfig(
        ...
        name="quota-test-worker",
        max_concurrency=1,
        simulation=True,
        registration_token=None,
        subscription_quotas=quota,
    )


def loop_credentials() -> WorkerCredentials:
    return WorkerCredentials(
        worker_id="worker-1",
        token="jeton-worker-tres-secret",
...
async def _fast_wait(stop: asyncio.Event, _delay: float) -> None:
    try:
...
async def test_the_loop_posts_each_batch_to_the_worker_route(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    posted: list[tuple[str, dict]] = []

    async def fake_collect(_config):
        return _static_reports()

    def handler(request: httpx.Request) -> httpx.Response:
        posted.append((request.url.path, json.loads(request.content)))
        return httpx.Response(200, json={"stored": 1, "ignored_older": 0, "removed": 0})

    monkeypatch.setattr(quotas, "collect_reports", fake_collect)
    stop = asyncio.Event()
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        task = asyncio.create_task(
            subscription_quota_loop(
                client,
                loop_worker_config(tmp_path, quota_config(tmp_path)),
                loop_credentials(),
                stop,
                WorkerLogger(state),
            )
    ...
    path, body = posted[0]
    assert path == "/work/workers/worker-1/subscription-quotas"
    SubscriptionQuotaBatch.model_validate(body)
    log = (state / "worker.log.jsonl").read_text(encoding="utf-8")
    assert "Quotas d'abonnement transmis" in log
    assert "claude_code=unavailable" in log


async def test_the_loop_survives_api_and_probe_failures_without_logging_secrets(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    calls: list[str] = []
    attempts = 0

    async def flaky_collect(_config):
        nonlocal attempts
        attempts += 1
        if attempts == 1:
            raise RuntimeError("jeton-worker-tres-secret dans un message imprévu")
        return _static_reports()

    def handler(request: httpx.Request) -> httpx.Response:
        calls.append(request.headers.get("authorization", ""))
        if len(calls) == 1:
            raise httpx.ConnectError("jeton-worker-tres-secret injoignable")
        return httpx.Response(503, json={"detail": "indisponible"})

    monkeypatch.setattr(quotas, "collect_reports", flaky_collect)
    monkeypatch.setattr(quotas, "_wait_or_stop", _fast_wait)
    stop = asyncio.Event()
    state = tmp_path / "state"
    headers = {"Authorization": "Bearer jeton-worker-tres-secret"}
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler), headers=headers) as client:
        task = asyncio.create_task(
            subscription_quota_loop(
                client,
                loop_worker_config(tmp_path, quota_config(tmp_path)),
    ...
    assert len(calls) >= 3
    log = (state / "worker.log.jsonl").read_text(encoding="utf-8")
    assert "jeton-worker-tres-secret" not in log
    assert "Relevé des quotas d'abonnement interrompu" in log
    assert "ConnectError" in log
    assert '"status_code": 503' in log


async def test_a_disabled_loop_never_calls_the_api(tmp_path: Path):
    calls: list[str] = []
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        await asyncio.wait_for(
            subscription_quota_loop(
                client,
                loop_worker_config(tmp_path, quota_config(tmp_path, enabled=False)),
                loop_credentials(),
                asyncio.Event(),
                WorkerLogger(tmp_path / "state"),
            ),
            timeout=5,
        )
    assert calls == []
...
def _client_factory(paths: list[str], seen: asyncio.Event | None = None):
    def factory(**kwargs):
        def handler(request: httpx.Request) -> httpx.Response:
            paths.append(request.url.path)
            if request.url.path.endswith("/subscription-quotas"):
                if seen is not None:
                    seen.set()
                return httpx.Response(200, json={"stored": 1, "ignored_older": 0, "removed": 0})
...
async def test_run_forever_starts_the_quota_loop_only_when_enabled(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    async def fake_collect(_config):
        return _static_reports()
    ...
    paths: list[str] = []
    seen = asyncio.Event()
    monkeypatch.setattr("acp_worker.main.httpx.AsyncClient", _client_factory(paths, seen))
    worker = asyncio.create_task(
        run_forever(loop_worker_config(tmp_path, quota_config(tmp_path)), loop_credentials())
    )
    try:
    ...
    assert "/work/workers/worker-1/subscription-quotas" in paths

    disabled_paths: list[str] = []
    monkeypatch.setattr("acp_worker.main.httpx.AsyncClient", _client_factory(disabled_paths))
    worker = asyncio.create_task(
        run_forever(
            loop_worker_config(tmp_path, quota_config(tmp_path, enabled=False)),
            loop_credentials(),
    ...
    try:
        for _ in range(500):
            if any(path.endswith("/claim") for path in disabled_paths):
                break
            await asyncio.sleep(0.01)
        # Une boucle démarrée enverrait son premier lot sans attendre l'intervalle.
        await asyncio.sleep(0.3)
    finally:
        worker.cancel()
        await asyncio.gather(worker, return_exceptions=True)
    assert any(path.endswith("/claim") for path in disabled_paths), disabled_paths
    assert not any(path.endswith("/subscription-quotas") for path in disabled_paths)
