"""Ligne d'état Claude Code livrée avec le worker (``python -m acp_worker.claude_statusline``).
...
import acp_worker
from acp_worker import claude_statusline as statusline
from acp_worker.subscription_quotas import SubscriptionQuotaConfig, probe_claude_code

WORKER_SOURCES = Path(acp_worker.__file__).resolve().parents[1]
...
def run_statusline(
    ...
    *arguments: str,
    ...
    return subprocess.run(
        [PYTHON, "-m", "acp_worker.claude_statusline", *arguments],
        input=stdin,
        capture_output=True,
        env=env,
        timeout=60,
