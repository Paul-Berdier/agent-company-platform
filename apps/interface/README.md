...
Sources TypeScript des deux greffons de tableau de bord qu'ACP livre dans son image Hermes
(étape P3 de la refonte, [docs/refonte/interface.md](../../docs/refonte/interface.md)) :
...
Aucun des deux n'a de code serveur : l'état vient du greffon `acp-poste` (`/v1/meta`,
`/v1/catalogue`) et des routes natives de Hermes (`/api/sessions`, `/api/skills`). React n'est
...
- **Aucun bouton qui fait semblant** : le Catalogue est en lecture seule.
- **Rendu par React uniquement** : ni `dangerouslySetInnerHTML`, ni `innerHTML`, ni `eval`, ni
  ...
  sur les sources ET les bundles).
